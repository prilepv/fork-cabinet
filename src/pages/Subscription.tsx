...
export default function Subscription() {
  ...
  // Device/traffic topup state (unified device management slider)
  const [showDeviceManagement, setShowDeviceManagement] = useState(false);
  const [targetDeviceLimit, setTargetDeviceLimit] = useState<number | null>(null);
  const [devicesToAdd, setDevicesToAdd] = useState(0);
  const [isDeviceSliderDragging, setIsDeviceSliderDragging] = useState(false);
  ...
  // Device price query (для итоговой цены, зависит от devicesToAdd)
  const { data: devicePriceData } = useQuery({
    ...
    enabled: showDeviceManagement && !!subscription,
  });

  // Отдельный запрос только для получения max_device_limit (чтобы слайдер не дёргался при смене devicesToAdd)
  const { data: deviceLimitInfo } = useQuery({
    queryKey: ['device-price-limit'],
  ...
  // Keep devicesToAdd in sync with target limit (only when increasing)
  useEffect(() => {
    if (!subscription || targetDeviceLimit === null) return;

    const currentLimit = deviceReductionInfo?.current_device_limit ?? subscription.device_limit;
    const diff = targetDeviceLimit - currentLimit;
    setDevicesToAdd(diff > 0 ? diff : 0);
  }, [targetDeviceLimit, subscription, deviceReductionInfo]);

  // Device reduction mutation
  const deviceReductionMutation = useMutation({
    mutationFn: () => {
      if (targetDeviceLimit === null) {
        throw new Error('No target device limit set');
      }
      return subscriptionApi.reduceDevices(targetDeviceLimit);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['subscription'] });
      queryClient.invalidateQueries({ queryKey: ['devices'] });
  ...
  // Derived device limits for unified management slider
  const currentDeviceLimit =
    (deviceReductionInfo && deviceReductionInfo.current_device_limit) ??
    subscription?.device_limit ??
    0;

  const minDeviceLimit =
    subscription && deviceReductionInfo
      ? Math.max(
  ...
  const maxDeviceLimit = deviceLimitInfo?.max_device_limit ?? currentDeviceLimit;
  const sliderMinDeviceLimit = minDeviceLimit;
  const sliderMaxDeviceLimit = Math.max(sliderMinDeviceLimit, maxDeviceLimit);

  // Инициализируем целевой лимит устройств при открытии панели
  useEffect(() => {
    if (!showDeviceManagement || !subscription) {
      if (!showDeviceManagement && targetDeviceLimit !== null) {
        setTargetDeviceLimit(null);
        setDevicesToAdd(0);
      }
      return;
    ...
    if (targetDeviceLimit === null) {
      const currentLimit =
        deviceReductionInfo?.current_device_limit ?? subscription.device_limit;
      setTargetDeviceLimit(currentLimit);
    }
  }, [showDeviceManagement, subscription, deviceReductionInfo, targetDeviceLimit]);

  const deviceSliderTrackRef = useRef<HTMLDivElement>(null);
  const updateDeviceLimitFromPosition = useCallback(
    (clientX: number) => {
      const el = deviceSliderTrackRef.current;
      if (!el) return;
      const rect = el.getBoundingClientRect();
      const totalSteps = sliderMaxDeviceLimit - sliderMinDeviceLimit;
      const innerWidth = rect.width;
      if (innerWidth <= 0 || totalSteps <= 0) return;
      const stepPx = innerWidth / totalSteps;
      const x = clientX - rect.left;
      const clampedX = Math.max(0, Math.min(innerWidth, x));
      const stepIndex = Math.round(clampedX / stepPx);
      const value = sliderMinDeviceLimit + stepIndex;
      setTargetDeviceLimit(
        Math.max(sliderMinDeviceLimit, Math.min(sliderMaxDeviceLimit, value)),
      );
    },
    [sliderMinDeviceLimit, sliderMaxDeviceLimit],
  );
      ...
      {subscription &&
        ...
        subscription.device_limit !== 0 && (
          ...
          >
            ...
            {/* Unified Device Management (slider) */}
            <div>
              {!showDeviceManagement ? (
                <button
                  ...
                  className={`group relative w-full overflow-hidden rounded-xl border p-4 text-left transition-all duration-300 ${
                    isDark
                      ? 'border-dark-700/50 bg-dark-800/50 hover:border-accent-500/60 hover:bg-dark-800'
                      : 'border-champagne-300/60 bg-champagne-200/40 hover:border-accent-500/60 hover:bg-white'
                  } active:scale-[0.98]`}
                >
                  <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <div className="absolute inset-[-1px] bg-gradient-to-r from-accent-500/25 via-emerald-400/20 to-sky-400/25 blur-[18px]" />
                  </div>
                  <div className="relative flex items-center justify-between">
                    <div>
                      <div className="font-medium text-dark-100">
                        {t('subscription.additionalOptions.manageDevices')}
                      </div>
                      <div className="mt-1 text-sm text-dark-400">
                        {t('subscription.additionalOptions.currentDeviceLimit', {
                          count: currentDeviceLimit || subscription.device_limit,
                        })}
                      </div>
                    </div>
                    <svg
                      className="h-5 w-5 text-dark-400 transition-transform duration-300 group-hover:translate-x-0.5"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={2}
                ...
                >
                  ...
                  {!subscription || !currentDeviceLimit ? (
                    ...
                    <div className="space-y-4">
                      ...
                      {sliderMaxDeviceLimit <= sliderMinDeviceLimit && (
                        <div className="rounded-lg bg-dark-800/40 p-3 text-center text-sm text-dark-400">
                          {t('subscription.additionalOptions.devicesUnavailable')}
                        </div>
                      )}

                      {sliderMaxDeviceLimit > sliderMinDeviceLimit && targetDeviceLimit !== null && (
                        <>
                          {(() => {
                            const clampedValue = Math.min(
                              sliderMaxDeviceLimit,
                              Math.max(sliderMinDeviceLimit, targetDeviceLimit),
                            );
                            const range = sliderMaxDeviceLimit - sliderMinDeviceLimit;
                            const stepCount = range;
                            const activeIndex = clampedValue - sliderMinDeviceLimit;
                            const fillRatio =
                              stepCount > 0 ? activeIndex / stepCount : 0;

                            const deviceLabel =
                              clampedValue === 1
                                ? t('subscription.additionalOptions.deviceOne')
                                : clampedValue >= 2 && clampedValue <= 4
                                  ? t('subscription.additionalOptions.deviceFew')
                                  : t('subscription.additionalOptions.deviceMany');

                            return (
                              <div className="rounded-2xl bg-dark-900/50 p-4">
                                <div className="mb-3 flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-dark-800 text-base font-semibold text-dark-50">
                                      {clampedValue}
                                    </div>
                                    <div className="text-sm font-semibold text-dark-50">
                                      {deviceLabel}
                                ...
                                <div className="mt-2">
                                  <div
                                    ref={deviceSliderTrackRef}
                                    className="relative flex cursor-grab touch-none select-none items-center py-2 active:cursor-grabbing"
                                    role="slider"
                                    aria-valuemin={sliderMinDeviceLimit}
                                    aria-valuemax={sliderMaxDeviceLimit}
                                    aria-valuenow={clampedValue}
                                    aria-label={t('subscription.additionalOptions.manageDevices')}
                                    tabIndex={0}
                                    onPointerDown={(e) => {
                                      setIsDeviceSliderDragging(true);
                                      (e.target as HTMLElement).setPointerCapture(e.pointerId);
                                      updateDeviceLimitFromPosition(e.clientX);
                                    }}
                                    onPointerMove={(e) => {
                                    ...
                                    onPointerUp={(e) => {
                                      setIsDeviceSliderDragging(false);
                                    ...
                                    onKeyDown={(e) => {
                                      if (e.key === 'ArrowLeft' && clampedValue > sliderMinDeviceLimit) {
                                        setTargetDeviceLimit(clampedValue - 1);
                                      } else if (
                                        e.key === 'ArrowRight' &&
                                        clampedValue < sliderMaxDeviceLimit
                                      ) {
                                        setTargetDeviceLimit(clampedValue + 1);
                                      }
                                    }}
                                  >
                                    <div className="absolute left-0 right-0 h-6 overflow-hidden rounded-full bg-dark-800">
                                      <div
                                        className="h-full rounded-full bg-gradient-to-r from-accent-500 to-purple-500 transition-[width] duration-150"
                                        style={{
                                          width: `calc(${fillRatio * 100}% + 28px * ${1 - fillRatio})`,
                                        }}
                                      />
                                    </div>
                                    <div className="relative z-10 flex h-6 w-full items-center justify-between px-1">
                                      {Array.from(
                                        { length: sliderMaxDeviceLimit - sliderMinDeviceLimit + 1 },
                                        (_, index) => {
                                          const value = sliderMinDeviceLimit + index;
                                          const isActive = value === clampedValue;
                                          const isLeft = value < clampedValue;
                                          ...
                                          return (
                                            ...
                                            >
                                              {isActive ? (
                                                <div
                                                  className="flex items-center justify-center rounded-full bg-dark-800 transition-all duration-150"
                                                  style={{
                                                    width: isDeviceSliderDragging ? 18 : 20,
                                                    height: isDeviceSliderDragging ? 18 : 20,
                                                  }}
                                                >
                                                  <div
                                                    className="h-3 w-3 rounded-full shrink-0"
                                                    style={{ backgroundColor: '#050109' }}
                                                  />
                                                </div>
                                              ) : (
                                                <div
                                                  ...
                                                  style={{
                                                    width: 7,
                                                    height: 7,
                                                    backgroundColor: isLeft ? '#050109' : '#233041',
                                                  }}
                                                />
                                              )}
                                            </div>
                          ...
                          <button
                            onClick={() => {
                              if (targetDeviceLimit === null) return;
                              if (targetDeviceLimit > currentDeviceLimit) {
                                devicePurchaseMutation.mutate();
                              } else if (targetDeviceLimit < currentDeviceLimit) {
                            ...
                            disabled={
                              ...
                              (targetDeviceLimit < currentDeviceLimit &&
                                (targetDeviceLimit < minDeviceLimit ||
                                  (deviceReductionInfo &&
                                    targetDeviceLimit < deviceReductionInfo.connected_devices_count)))
                            }
                            className="btn-primary w-full py-3"
                          >
                            {devicePurchaseMutation.isPending || deviceReductionMutation.isPending ? (
                          ...
                          {(devicePurchaseMutation.isError || deviceReductionMutation.isError) && (
                            <div className="text-center text-sm text-error-400">
                              {getErrorMessage(
                                devicePurchaseMutation.error || deviceReductionMutation.error,
                              )}
                            </div>
                          )}
                        </>
